...
const SalesTable = (salesdata) => {
  ...
  const COLUMNS = [
...

    {
      ...
      accessor: "subcatego",
      Cell: (row) => {
        if (row?.cell?.value[0] === undefined) {
          return <span> - </span>;
        }
        else {
          return (
            <div>
              {row?.cell?.value.map((item, i) => (
    ...
    {/* MODAL DELETE  */}
      ...
      >
        ...
        <h4 className="font-medium text-lg mb-3 text-slate-900">
          etes vous sur de vouloir supprimer {ModalDeleteData?.nom}  .
        </h4>
        <div className="text-base text-slate-600 dark:text-slate-300">
          Sous-categories : {ModalDeleteData?.subcatego?.map((item, i) => ( <span key={i} className="block">{item ? item : "-"}</span>))}
        </div>
        <div className="text-base text-slate-600 dark:text-slate-300">
